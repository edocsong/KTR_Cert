...
export default function ItemDetailPage() {
  ...
  return (
    <div className="animate-fade-up mx-auto max-w-4xl px-4 py-10 sm:px-6">
      <Link
        to="/"
      ...
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        {item.certTypes.map((type) => (
          <span
            key={type}
            className="rounded-full bg-brand-50 px-2.5 py-0.5 text-xs font-medium text-brand-500"
          >
            {type}
          </span>
        ))}
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex items-baseline justify-between bg-brand-500 px-6 py-4">
              <dt className="text-sm font-semibold text-white">
                예상 총 비용
              </dt>
              <dd className="text-lg font-bold text-accent-400">
                {formatWon(totalFee(item.fee))}
              </dd>
            </div>
            <details className="group px-6 py-3 text-xs text-slate-500">
          ...
          <Link
            ...
            className="rounded-lg bg-brand-500 px-4 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-brand-600 active:scale-[0.98]"
          >
            이 품목으로 상담 문의하기
          </Link>
        </div>
